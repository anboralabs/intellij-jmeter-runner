...
public class CheckLicense {
   ...
   */
  private static final String PRODUCT_CODE = "PJMETERRUNNER";
  ...
  private static X509Certificate createCertificate(byte[] certBytes, Collection<byte[]> intermediateCertsBytes, boolean checkValidityAtCurrentDate) throws Exception {
    ...
    for (byte[] bytes : intermediateCertsBytes) {
            allCerts.add(x509factory.generateCertificate(
          ...
          try {
            ...
            if (!checkValidityAtCurrentDate) {
              ...
              // period, so that we do not depend on the actual moment when the
              // check is performed
            ...
            final CertPath path = CertPathBuilder.getInstance("PKIX")
                                      .build(pkixParams)
                                      .getCertPath();
            if (path != null) {
